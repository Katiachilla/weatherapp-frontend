fetch('https://weatherapp-backend-f11etcgfy-katiachilla.vercel.app/weather')
...
function updateDeleteCityEventListener() {
	for (let i = 0; i < document.querySelectorAll('.deleteCity').length; i++) {
		document.querySelectorAll('.deleteCity')[i].addEventListener('click', function () {
			fetch(`https://weatherapp-backend-f11etcgfy-katiachilla.vercel.app/weather/${this.id}`, { method: 'DELETE' })
				.then(response => response.json())
				.then(data => {
					if (data.result) {
